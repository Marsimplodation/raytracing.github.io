    ...
    void render() {
        ...
        tbb::parallel_for(0, image_height, [&](int j){
            for (int i = 0; i < image_width; i++) {
                color & pixel_color = pixels[j * image_width + i];
        ...
        std::string output;
        output.reserve(pixels.size() * 12); 
        // Rough estimate: "255 255 255\n" ≈ 12 chars per pixel

        for (const auto& color : pixels) {
            float r = std::sqrtf(pixel_samples_scale * color.x);
            float g = std::sqrtf(pixel_samples_scale * color.y);
            float b =  std::sqrtf(pixel_samples_scale * color.z);

            int rbyte = int(256 * clamp(r, 0.0f, 0.999f));
            int gbyte = int(256 * clamp(g, 0.0f, 0.999f));
            int bbyte = int(256 * clamp(b, 0.0f, 0.999f));

            output += std::to_string(rbyte);
            output += ' ';
            output += std::to_string(gbyte);
            output += ' ';
            output += std::to_string(bbyte);
            output += '\n';
        }
